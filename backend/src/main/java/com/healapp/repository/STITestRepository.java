...
public interface STITestRepository extends JpaRepository<STITest, Long> {
        ...
        List<STITest> findByConsultantId(Long consultantId); // Tìm theo dịch vụ

        List<STITest> findByStiService(STIService stiService);

        List<STITest> findByStiServiceId(Long serviceId);
        ...
        @Query("SELECT t FROM STITest t " +
                        ...
                        "WHERE t.status IN ('SAMPLED', 'RESULTED', 'COMPLETED') " +
                        "ORDER BY t.updatedAt DESC")
        List<STITest> findAllConsultantAccessibleTests(); // Method for rating eligibility check

        @Query("SELECT t FROM STITest t WHERE t.customer.id = :customerId " +
                        "AND t.stiService.id = :serviceId AND t.status = :status")
        List<STITest> findByCustomerIdAndStiServiceIdAndStatus(
