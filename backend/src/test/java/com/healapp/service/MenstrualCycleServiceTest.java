...
class MenstrualCycleServiceTest {
    ...
    void getAllMenstrualCycle_UserNotFound_ShouldFail() {
        ...
        assertFalse(response.isSuccess());
        assertTrue(response.getMessage().contains("Lấy tất cả chu kỳ kinh nguyệt thất bại"));
        verify(userRepository).findById(1L);
    }
    ...
    @Test
    @DisplayName("Gửi email nhắc nhở ngày rụng trứng thành công")
    void sendOvulationReminderEmail_Success() {
        // Set ovulation date để hôm nay là 1 ngày trước rụng trứng
        sampleMenstrualCycle.setOvulationDate(LocalDate.now().plusDays(1));
        
        when(menstrualCycleRepository.findLatestCycleBeforeToday(1L, LocalDate.now()))
            .thenReturn(Optional.of(sampleMenstrualCycle));
        doNothing().when(emailService).sendOvulationReminderAsync(anyString(), anyString(), any(LocalDate.class));

        ApiResponse<String> response = menstrualCycleService.sendOvulationReminderEmail();

        assertTrue(response.isSuccess());
        assertTrue(response.getMessage().contains("Đã gửi email nhắc nhở ngày rụng trứng cho user"));

        verify(menstrualCycleRepository).findLatestCycleBeforeToday(1L, LocalDate.now());
        verify(emailService).sendOvulationReminderAsync(anyString(), anyString(), any(LocalDate.class));
    }

    @Test
    @DisplayName("Gửi email nhắc nhở ngày rụng trứng thất bại - không tìm thấy chu kỳ")
    void sendOvulationReminderEmail_NoCycle_ShouldFail() {
        when(menstrualCycleRepository.findLatestCycleBeforeToday(1L, LocalDate.now()))
        ...
        assertFalse(response.isSuccess());
        assertTrue(response.getMessage().contains("Không tìm thấy chu kỳ kinh nguyệt cho user"));

        verify(menstrualCycleRepository).findLatestCycleBeforeToday(1L, LocalDate.now());
        verify(emailService, never()).sendOvulationReminderAsync(anyString(), anyString(), any(LocalDate.class));
    }
    ...
    void sendOvulationReminderEmail_ReminderDisabled_ShouldFail() {
        ...
        assertFalse(response.isSuccess());
        assertTrue(response.getMessage().contains("Nhắc nhở không được bật cho user"));
    ...
    @Test
    @DisplayName("Gửi email nhắc nhở tỉ lệ mang thai thành công")
    void sendReminderPregnency_Success() {
        List<UserDtls> users = Arrays.asList(sampleUser);
        List<PregnancyProbLog> logs = Arrays.asList(samplePregnancyProbLog);
        
        // Set date để thỏa mãn điều kiện today.isAfter(start) && today.isBefore(end)
        samplePregnancyProbLog.setDate(LocalDate.now().minusDays(5));
        PregnancyProbLog log2 = new PregnancyProbLog();
        log2.setId(2L);
        log2.setMenstrualCycle(sampleMenstrualCycle);
        log2.setDate(LocalDate.now().plusDays(5));
        log2.setProbability(25.0);
        List<PregnancyProbLog> logsWithRange = Arrays.asList(samplePregnancyProbLog, log2);
        
        when(userRepository.findAll()).thenReturn(users);
        when(menstrualCycleRepository.findLatestCycleBeforeToday(1L, LocalDate.now()))
            .thenReturn(Optional.of(sampleMenstrualCycle));
        when(pregnancyProbLogRepository.findAllByMenstrualCycleId(1L))
            .thenReturn(Optional.of(logsWithRange));
        doNothing().when(emailService).sendOvulationWithPregnancyProbReminderAsync(
            anyString(), anyString(), anyInt(), anyDouble(), any(LocalDate.class));

        // Gọi method private thông qua reflection
        ReflectionTestUtils.invokeMethod(menstrualCycleService, "sendReminderPregnency");

        verify(userRepository).findAll();
        verify(menstrualCycleRepository).findLatestCycleBeforeToday(1L, LocalDate.now());
        verify(pregnancyProbLogRepository).findAllByMenstrualCycleId(1L);
        verify(emailService).sendOvulationWithPregnancyProbReminderAsync(
            anyString(), anyString(), anyInt(), anyDouble(), any(LocalDate.class));
    }

    @Test
    @DisplayName("Gửi email nhắc nhở tỉ lệ mang thai - không có chu kỳ")
    void sendReminderPregnency_NoCycle_ShouldSkip() {
        List<UserDtls> users = Arrays.asList(sampleUser);
        ...
        verify(menstrualCycleRepository).findLatestCycleBeforeToday(1L, LocalDate.now());
        verify(pregnancyProbLogRepository, never()).findAllByMenstrualCycleId(any());
        verify(emailService, never()).sendOvulationWithPregnancyProbReminderAsync(
            anyString(), anyString(), anyInt(), anyDouble(), any(LocalDate.class));
    }
    ...
    @Test
    @DisplayName("Gửi email nhắc nhở tỉ lệ mang thai - exception xử lý riêng lẻ")
    void sendReminderPregnency_Exception_ShouldHandleIndividually() {
        List<UserDtls> users = Arrays.asList(sampleUser);
        ...
        // Method này không nên throw exception mà chỉ log lỗi
        assertDoesNotThrow(() -> {
            ReflectionTestUtils.invokeMethod(menstrualCycleService, "sendReminderPregnency");
        });
