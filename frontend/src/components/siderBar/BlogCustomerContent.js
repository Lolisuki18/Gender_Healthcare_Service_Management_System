...
const EditBlogDialog = ({ open, onClose, blog, onSaved }) => {
        ...
        sections: blog.sections
          ? blog.sections.map((s) => ({
              ...
              existingSectionImage:
                s.sectionImage || s.existingSectionImage || '',
    ...
    if (file) {
      ...
      reader.onload = (e) =>
        setForm((prev) => ({
          ...prev,
          sections: prev.sections.map((s, i) =>
            i === idx ? { ...s, sectionImage: e.target.result } : s
          ),
        }));
      reader.readAsDataURL(file);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const sectionImages = [],
        sectionImageIndexes = [];
      Object.keys(sectionFiles).forEach((idx) => {
        sectionImages.push(sectionFiles[idx]);
        sectionImageIndexes.push(parseInt(idx));
      });
      sectionImages.forEach((f) => formData.append('sectionImages', f));
      sectionImageIndexes.forEach((i) =>
        formData.append('sectionImageIndexes', i)
      );
      const requestData = {
        title: form.title,
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle
        sx={{ fontWeight: 700, color: 'primary.main', fontSize: '1.3rem' }}
      >
        Chỉnh sửa bài viết
      </DialogTitle>
            ...
            <Card key={idx} sx={{ mb: 2, p: 2, background: '#f8fbff' }}>
              ...
              <Box>
                <input
                  accept="image/*"
                  style={{ display: 'none' }}
                  id={`edit-section-image-${idx}`}
                  type="file"
                  onChange={(e) =>
                    handleSectionImageChange(idx, e.target.files[0])
                  }
                />
                <label htmlFor={`edit-section-image-${idx}`}>
                  <Button
                    variant="outlined"
                    component="span"
                    startIcon={<CloudUploadIcon />}
                  ...
                  <Box sx={{ mt: 1 }}>
                    {(() => {
                      ...
                      if (
                        ...
                        section.sectionImage.startsWith('data:')
                      ) {
              ...
              >
                ...
                <CardContent sx={{ flex: 1 }}>
                  <Typography
                    variant="h6"
                    fontWeight={700}
                    sx={{ mb: 1, color: 'primary.main', cursor: 'pointer' }}
