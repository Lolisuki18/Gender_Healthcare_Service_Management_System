...
const MyReviewsContent = () => {
  ...
  const fetchReviewsData = useCallback(async () => {
    ...
    try {
      ...
      notify.error('Lỗi', 'Đã xảy ra lỗi khi tải dữ liệu đánh giá');
    } finally {
      setLoading(false);
    }
  ...
  const fetchStatisticsData = useCallback(async () => {
    ...
    try {
      const response =
        await reviewService.getConsultantRatingSummary(consultantId);

      if (response) {
        setStatistics({
          averageRating: response.averageRating || 0,
          totalReviews: response.totalRatings || 0,
          ratingDistribution: response.starDistribution || {
            1: 0,
            2: 0,
            3: 0,
            4: 0,
          ...
          <Grid item xs={12} md={6}>
            <Box
              sx={{
                ...
                px: 2,
                bgcolor:
                  'linear-gradient(135deg, rgba(74, 144, 226, 0.05) 0%, rgba(102, 126, 234, 0.05) 100%)',
                borderRadius: '16px',
            ...
            >
              ...
              <Typography
                variant="h6"
                sx={{ color: '#4A90E2', fontWeight: 600, mb: 1 }}
              >
                Điểm trung bình
              </Typography>
          ...
          <Grid item xs={12} md={6}>
            <Box sx={{ px: 2 }}>
              ...
              {[5, 4, 3, 2, 1].map((rating) => (
                ...
                >
                  <Box
                    sx={{ display: 'flex', alignItems: 'center', minWidth: 60 }}
                  >
                    <Typography
                      sx={{
                        mr: 1,
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Avatar
                        src={
                          review.userAvatar &&
                          review.userAvatar !== '/img/avatar/default.jpg'
                            ? review.userAvatar
                            : undefined
                        }
                        sx={{
                        ...
                        icon={
                          <StarIcon
                            ...
                            sx={{
                              color: '#FFB400',
                              filter:
                                'drop-shadow(0 1px 2px rgba(255, 180, 0, 0.3))',
                            }}
                          />
                        }
                        emptyIcon={
