      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, mt: 'auto' }}>
        <Box
          sx={{
            ...
            height: 8,
            borderRadius: '50%',
            backgroundColor: change?.includes('+') ? '#10b981' : change?.includes('-') ? '#ef4444' : '#64748b',
            flexShrink: 0,
            mt: 0.5,
          }}
        />
        <Typography
          variant="caption"
          sx={{ 
            color: change?.includes('+') ? '#10b981' : change?.includes('-') ? '#ef4444' : '#64748b',
            fontWeight: 600,
...
const DashboardContent = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        ...
        const activeSTIServicesResponse = await adminService.getActiveSTIServices();
        ...
        const allConsultationsResponse = await adminService.getAllConsultations();
        ...
        const calculateUserGrowthRate = (users) => {
          ...
          const growthRate = (currentMonthUsers / usersBeforeThisMonth * 100).toFixed(1);
        ...
        const calculateSTIServiceGrowthRate = (services) => {
          ...
          const growthRate = (currentMonthServices / servicesBeforeThisMonth * 100).toFixed(1);
          return growthRate > 0 ? `+${growthRate}%` : `${growthRate}%`;
        };

        // Tính toán tỉ lệ tăng trưởng cho lịch hẹn hoàn thành (STI tests + consultations)
        const calculateCompletedAppointmentsGrowthRate = (stiTests, consultations) => {
          if ((!stiTests || !Array.isArray(stiTests)) && (!consultations || !Array.isArray(consultations))) return '+0%';
          ...
          const completedSTITests = (stiTests || []).filter(test => 
          ...
          const completedConsultations = (consultations || []).filter(consultation => 
            consultation.status === 'COMPLETED' || consultation.status === 'completed'
          );
          ...
          const growthRate = (currentMonthCompleted / appointmentsBeforeThisMonth * 100).toFixed(1);
          return growthRate > 0 ? `+${growthRate}%` : `${growthRate}%`;
        };
        ...
        const calculateMonthlyStats = (users, stiTests, consultations) => {
          ...
          for (let month = 0; month < 12; month++) {
            ...
            const monthUsers = (users || []).filter(user => {
              if (!user.createdDate) return false;
              const createdDate = new Date(user.createdDate);
              return createdDate.getMonth() === month && createdDate.getFullYear() === currentYear;
            }).length;
        ...
        const calculateGrowthRate = (current, mockPrevious) => {
          ...
          const rate = ((current - mockPrevious) / mockPrevious * 100).toFixed(1);
          return rate > 0 ? `+${rate}%` : `${rate}%`;
        };
        
        // Tính tổng dịch vụ STI hoạt động thực tế
        const activeSTIServicesCount = activeSTIServicesResponse?.data?.length || 0;
        ...
        const realSTIServiceGrowthRate = calculateSTIServiceGrowthRate(activeSTIServicesResponse?.data || []);
        
        // Mock dữ liệu tháng trước cho các metrics khác
        const mockPreviousMonth = {
          activeDoctors: Math.floor((dashboardResponse?.activeDoctors || 0) * 0.95),
          totalConsultations: Math.floor((dashboardResponse?.totalConsultations || 0) * 0.88),
        };
        
        // Tính tỉ lệ tăng trưởng người dùng thực tế
        const realUserGrowthRate = calculateUserGrowthRate(usersResponse?.data || []);
        
        // Tính tổng appointments hoàn thành
        const completedAppointments = [
          ...(allSTITestsResponse?.data || []).filter(test => (test.status === 'COMPLETED' || test.status === 'completed')),
          ...(allConsultationsResponse?.data || []).filter(consultation => (consultation.status === 'COMPLETED' || consultation.status === 'completed'))
        ].length;
        
        // Tính tỉ lệ tăng trưởng appointments hoàn thành
        const appointmentsGrowthRate = calculateCompletedAppointmentsGrowthRate(allSTITestsResponse?.data || [], allConsultationsResponse?.data || []);
        
        // Tính toán dữ liệu thống kê theo tháng cho biểu đồ
        const monthlyStatsData = calculateMonthlyStats(usersResponse?.data || [], allSTITestsResponse?.data || [], allConsultationsResponse?.data || []);
        ...
        // Xử lý dữ liệu thực
        const processedRecentUsers = processRecentUsers(usersResponse?.data || []);
        ...
        setDashboardData({
          ...
          doctorGrowthRate: calculateGrowthRate(dashboardResponse?.activeDoctors || 0, mockPreviousMonth.activeDoctors),
          consultationGrowthRate: calculateGrowthRate(dashboardResponse?.totalConsultations || 0, mockPreviousMonth.totalConsultations),
  ...
  if (loading) return (
    <Box sx={{ 
      ...
      <Typography variant="h4" sx={{ mb: 2 }}>Đang tải tổng quan...</Typography>
            ...
            <Grid item xs={12} sm={6} lg={3} key={item}>
              <Card sx={{ minHeight: '180px', p: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                  <Box sx={{ width: 60, height: 60, bgcolor: '#f0f0f0', borderRadius: 3, mr: 2 }} />
                  <Box sx={{ flex: 1 }}>
                    <Box sx={{ height: 24, bgcolor: '#f0f0f0', borderRadius: 1, mb: 1 }} />
                    <Box sx={{ height: 16, bgcolor: '#f0f0f0', borderRadius: 1, width: '60%' }} />
                  </Box>
                </Box>
                <Box sx={{ height: 12, bgcolor: '#f0f0f0', borderRadius: 1, width: '80%' }} />
              </Card>
            </Grid>
          ))}
        </Grid>
      ...
      <Box sx={{ width: '100%', mb: 4 }}>
        <Grid container spacing={2} sx={{ maxWidth: '100%' }}>
          ...
          <Grid item xs={12} sm={6} lg={3} sx={{ display: 'flex' }}>
            <StatCard
              title="Tổng lịch hẹn"
              value={(dashboardData.totalAppointments || 0).toLocaleString()}
              change={`${dashboardData.appointmentsGrowthRate || '+0%'} so với tháng trước`}
              icon={CalendarIcon}
              color="#7B61FF"
            />
          </Grid>
          <Grid item xs={12} sm={6} lg={3} sx={{ display: 'flex' }}>
            <StatCard
              ...
              change={dashboardData.serviceStatusInfo || '0/0 dịch vụ hoạt động'}
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          <Card sx={{ 
            ...
            <Typography variant="h5" sx={{ mb: 3, fontWeight: 700, color: '#1a202c' }}>
            ...
            <Box sx={{ height: '530px', width: '100%' }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlyData} width="100%">
                  ...
                  />
                  <Bar dataKey="users" fill="#4A90E2" name="Người dùng mới" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="appointments" fill="#E91E63" name="Lịch hẹn" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </Box>
          </Card>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                >
                  <Box sx={{ textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    >
                      <PersonAddIcon sx={{ color: quickActions[0].color, fontSize: 24 }} />
                    </Box>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem', color: '#1a202c', mb: 0.5 }}>
                ...
                >
                  <Box sx={{ textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    >
                      <MedicalIcon sx={{ color: quickActions[1].color, fontSize: 24 }} />
                    </Box>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem', color: '#1a202c', mb: 0.5 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                >
                  <Box sx={{ textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    >
                      <CalendarIcon sx={{ color: quickActions[2].color, fontSize: 24 }} />
                    </Box>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem', color: '#1a202c', mb: 0.5 }}>
                ...
                >
                  <Box sx={{ textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    >
                      <AssessmentIcon sx={{ color: quickActions[3].color, fontSize: 24 }} />
                    </Box>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem', color: '#1a202c', mb: 0.5 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                >
                  <Box sx={{ textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    >
                      <SettingsIcon sx={{ color: quickActions[4].color, fontSize: 24 }} />
                    </Box>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem', color: '#1a202c', mb: 0.5 }}>
                ...
                >
                  <Box sx={{ textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    >
                      <ArticleIcon sx={{ color: quickActions[5].color, fontSize: 24 }} />
                    </Box>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem', color: '#1a202c', mb: 0.5 }}>
                  ...
                  <TableRow key={user.id} sx={{ '&:hover': { backgroundColor: '#f8fafc' } }}>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        <Avatar 
                          sx={{ 
                            ...
                            fontSize: '0.85rem',
                            fontWeight: 600,
                            background: 'linear-gradient(135deg, #4A90E2, #00C9A7)',
                          }}
                        >
                          {user.avatar}
                        </Avatar>
                    ...
                    <TableCell>
                      <Chip
                        ...
                        size="small"
                        sx={{
                          backgroundColor: user.role === 'Quản lý' ? '#4A90E2' : 
                                         user.role === 'Tư vấn viên' ? '#00C9A7' : '#64748b',
                          color: '#fff',
                          fontWeight: 600,
                          fontSize: '0.7rem',
                        }}
                    ...
                    <TableCell>
                      <Chip
                        ...
                        variant="outlined"
                        sx={{
                          borderColor: user.status === 'Hoạt động' ? '#10b981' : '#ef4444',
