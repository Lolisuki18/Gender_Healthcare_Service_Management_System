...
import paymentInfoService from '@/services/paymentInfoService'; // Service gọi API thông tin thanh toán
...
import PaymentSection from '@/components/TestRegistration/PaymentSection'; // Component chọn phương thức thanh toán
...
const TIME_SLOTS = [
  ...
  '09:00',
  '10:00',
  '11:00', // Khung giờ buổi sáng
  '12:00',
  '13:00',
  '14:00',
  '15:00', // Khung giờ buổi chiều
  '16:00',
  '17:00', // Khung giờ buổi tối
];
    ...
    const fetchServices = async () => {
      try {
        ...
        if (
          ...
          console.warn(
            'Services response failed or no data:',
            servicesResponse
          ); // Log cảnh báo
        ...
        if (
          ...
          console.warn(
            'Packages response failed or no data:',
            packagesResponse
          ); // Log cảnh báo
...
const useSearch = (singleTests, packages) => {
  ...
  useEffect(() => {
    ...
    const filteredSingle = singleTests.filter(
      (test) =>
...
const extractTestMetrics = (service) => {
    ...
    service.components.forEach((component) => {
      ...
      if (testName && unit) {
        ...
        const referenceRange =
          component.normalRange || component.referenceRange || 'N/A';
  ...
  if (service.name?.toLowerCase().includes('hiv')) {
    ...
    service.name?.toLowerCase().includes('giang mai') ||
    service.name?.toLowerCase().includes('syphilis')
  ) {
    ...
    service.name?.toLowerCase().includes('lậu') ||
    service.name?.toLowerCase().includes('gonorrhea')
  ) {
    ...
    service.name?.toLowerCase().includes('herpes') ||
    service.name?.toLowerCase().includes('hsv')
  ) {
    ...
    service.name?.toLowerCase().includes('viêm gan') ||
    service.name?.toLowerCase().includes('hepatitis')
  ) {
...
const getPackageStats = (packageData) => {
  ...
  const savings =
    packagePrice > 0 && totalServicePrice > packagePrice
      ? Math.round(
          ((totalServicePrice - packagePrice) / totalServicePrice) * 100
        ) // Công thức tính % tiết kiệm
      : Math.round(
          (1 - packagePrice / Math.max(totalServicePrice, packagePrice)) * 100
...
function TestRegistrationPage() {
  ...
  useEffect(() => {
    ...
    if (location.state?.selectedPackage && packages.length > 0) {
      ...
      const idx = packages.findIndex(
        (pkg) => pkg.id === location.state.selectedPackage.id
      );
      if (idx !== -1) {
        ...
        if (
          location.state.skipServiceSelection ||
          location.state.initialStep === 1 ||
          location.state.activeStep === 1
        ) {
    ...
    if (location.state?.selectedTest && singleTests.length > 0) {
      ...
      const idx = singleTests.findIndex(
        (test) => test.id === location.state.selectedTest.id
      );
      if (idx !== -1) {
  ...
  const singleTotalPages = Math.ceil(
    filteredSingleTests.length / ITEMS_PER_PAGE
  );
  ...
  const paginatedSingleTests = useMemo(() => {
    ...
    const slicedTests = filteredSingleTests.slice(
      start,
  ...
  const paginatedPackages = useMemo(() => {
    ...
    const slicedPackages = filteredPackages.slice(
      start,
      start + ITEMS_PER_PAGE
    );
  ...
  /**
   * Xử lý khi chọn thẻ thanh toán
   * @param {number} cardId - ID của thẻ được chọn
   */
  ...
  const handleBookTest = async () => {
    ...
    if (!isValidAppointmentTime(selectedDate, selectedTime)) {
      if (!selectedDate || !selectedTime) {
        ...
        toast.error(
          'Không thể đặt lịch cho khung giờ đã qua hoặc quá gần giờ hiện tại. Vui lòng chọn khung giờ cách ít nhất 2 tiếng từ bây giờ!'
        );
      }
      return;
    }
    ...
    if (paymentMethod === 'card' && selectedCard) {
      try {
        ...
        const cardResponse =
          await paymentInfoService.getForPayment(selectedCard);
        ...
        toast.error('Không thể lấy thông tin thẻ. Vui lòng thử lại.');
        setIsBooking(false);
        return;
      }
    }
    ...
    try {
      setIsBooking(true);
      console.log(
        'Gửi request đặt lịch với payload:',
      ...
      if (res.data.testId && res.data.paymentStatus === 'FAILED') {
        ...
        const paymentMessage =
          res.data.message || 'Thanh toán thất bại, bạn có thể thử lại sau.';
        ...
        setStripePaymentIntentId(
          res.data.stripePaymentIntentId ||
            res.data.data?.stripePaymentIntentId ||
            null
        );
        setStripeReceiptUrl(
          res.data.stripeReceiptUrl || res.data.data?.stripeReceiptUrl || null
        );
      } else if (
        ...
        toast.success(
          'Đặt lịch thành công!',
          res.data.message || detailedMessage
        );
      ...
      if (err?.response?.data?.message) {
        ...
        errorMessage =
          'Dữ liệu không hợp lệ: ' + JSON.stringify(err.response.data.errors);
      } else if (err?.message) {
  ...
  return (
    <Box
      sx={{
        background:
          'linear-gradient(135deg, #f8faff 0%, #e6f3ff 30%, #f0f7ff 70%, #ffffff 100%)',
        minHeight: '100vh',
        position: 'relative',
        overflow: 'hidden',
      }}
    >
      ...
      >
                ...
                >
                  ...
                  <Box
                    sx={{
                      ...
                      transform:
                        index === activeStep ? 'scale(1.03)' : 'scale(1)',
                      cursor: 'default',
                      '&:hover':
                        index > activeStep
                          ? {
                  ...
                  >
                    {Array.from({ length: singleTotalPages }, (_, i) => (
                      <Button
                        key={i + 1}
                        variant={
                          singlePage === i + 1 ? 'contained' : 'outlined'
                        }
                        color="primary"
                        sx={{
                          minWidth: 40,
                  ...
                  >
                    {Array.from({ length: packageTotalPages }, (_, i) => (
                      <Button
                        key={i + 1}
                        variant={
                          packagePage === i + 1 ? 'contained' : 'outlined'
                        }
                        color="primary"
                        sx={{
                          minWidth: 40,
                ...
                {paymentFailed && (
                  ...
                  >
                    ...
                    <Box>
                      ...
                      >
                        💡 Bạn có thể thử lại với phương thức thanh toán khác
                        hoặc liên hệ hỗ trợ
                      </Typography>
                    </Box>
                  </Box>
        ...
        <BookingSuccessDialog
          ...
          onViewBookings={() => navigate('/profile?tab=appointments')}
