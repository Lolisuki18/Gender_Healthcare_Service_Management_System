      ...
      <Grid container spacing={3}>
        {services.map((service, idx) => (
          <Grid item xs={12} sm={6} md={4} key={service.id} display="flex" justifyContent="center">
            <Card sx={{ borderRadius: 3, boxShadow: 2, width: '100%', maxWidth: 320, mx: 'auto', display: 'flex', flexDirection: 'column', height: '100%' }}>
              <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                <Box sx={{ height: 120, bgcolor: '#f4f8fc', borderRadius: 2, mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Typography color="#bdbdbd">Ảnh dịch vụ</Typography>
                </Box>
                <Typography variant="h6" fontWeight={700} mb={1}>{service.title}</Typography>
                <Typography color="text.secondary" mb={2}>{service.shortDesc}</Typography>
              ...
              <Box sx={{ p: 2, pt: 0, textAlign: 'center' }}>
                <Button variant="outlined" fullWidth onClick={() => navigate(service.detailRoute)}>
                  Xem chi tiết
                </Button>
              </Box>
            </Card>
