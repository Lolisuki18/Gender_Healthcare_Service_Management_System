...
const OvulationPage = ({ stats }) => {
  ...
  const getAverageCycleLengthOfCurrentCycles = (menstrualCycles) => {
    ...
    try {
      ...
      const total = recentCycles.reduce((sum, cycle) => sum + (cycle.cycleLength || 0), 0);
  ...
  const averageCycleLengthOfCurrentCycles = getAverageCycleLengthOfCurrentCycles(menstrualCycles);

  const getConsistency = (menstrualCycles) => {
    console.log('🔍 [getConsistency] Input data:', menstrualCycles);
    
    if (!Array.isArray(menstrualCycles) || menstrualCycles.length < 3) {
      console.log('❌ [getConsistency] Không đủ dữ liệu, cần ít nhất 3 chu kỳ. Hiện có:', menstrualCycles?.length || 0);
      return 'unknown';
    }
    ...
    try {
      ...
      for (const cycle of recentCycles) {
        ...
        if (!cycle.numberOfDays || cycle.numberOfDays <= 2 || cycle.numberOfDays > 7) {
          console.log('❌ [getConsistency] Chu kỳ không đều - số ngày kinh không hợp lệ:', cycle.numberOfDays);
          return 'irregular';
        }
        
        // Kiểm tra độ dài chu kỳ có tồn tại
        if (!cycle.cycleLength || typeof cycle.cycleLength !== 'number') {
          console.log('❌ [getConsistency] Dữ liệu không đầy đủ - thiếu cycleLength:', cycle.cycleLength);
          return 'unknown';
        }
      }
      ...
      for (let i = 0; i < cycleLengths.length - 1; i++) {
        ...
        console.log(`🔢 [getConsistency] So sánh chu kỳ ${i} và ${i+1}: ${cycleLengths[i]} vs ${cycleLengths[i + 1]} = chênh lệch ${difference} ngày`);
        if (difference > maxDifference) {
          maxDifference = difference;
        }
      }
      
      console.log('📈 [getConsistency] Chênh lệch tối đa:', maxDifference, 'ngày');
  ...
  const getAdvice = () => {
    ...
    if (averageCycleLengthOfCurrentCycles < 21 && averageCycleLengthOfCurrentCycles !== null) {
      advice.push({
  ...
  // Pagination logic
  const sortedCycles = menstrualCycles.sort((a, b) => new Date(b.startDate) - new Date(a.startDate));
  ...
  const handlePageChange = (page) => {
    ...
    setTimeout(() => {
      const cycleListElement = document.querySelector('[data-section="cycle-list"]');
      if (cycleListElement) {
  ...
  const handleSubmitCycle = async (data) => {
    try {
      ...
      alert(data.saveToDatabase ? 'Ghi nhận chu kỳ thất bại!' : 'Tính toán thất bại!');
    }
  };
  ...
  const calculateCycleData = (data) => {
    ...
    ovulationDate.setDate(startDate.getDate() + Math.floor(data.cycleLength) - 14);
    ...
    return {
      startDate: startDate,
      endDate: new Date(startDate.getTime() + (data.numberOfDays - 1) * 24 * 60 * 60 * 1000),
  ...
  const handleDeleteCycle = async (cycle) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa chu kỳ #${menstrualCycles.length - menstrualCycles.indexOf(cycle)} không? Hành động này không thể hoàn tác.`)) {
      try {
        ...
        const newTotalPages = Math.ceil((menstrualCycles.length - 1) / itemsPerPage);
              ...
              <Box sx={{ marginBottom: 4 }} data-section="cycle-list">
                <Card className={styles.cycleListCard}>
                  <Box className={styles.cycleListHeader}>
                    ...
                    <Box>
                      <Typography variant="h6" className={styles.cycleListTitle}>
                  ...
                  {menstrualCycles.length > 0 ? (
                    <>
                      <Box className={styles.cycleListContainer}>
                        {currentCycles.map((cycle, index) => (
                          <Card key={startIndex + index} className={styles.cycleItem}>
                            <Box className={styles.cycleItemHeader}>
                              ...
                              <Box className={styles.cycleItemInfo}>
                                <Typography className={styles.cycleItemTitle}>
                                  Chu kỳ #{menstrualCycles.length - (startIndex + index)}
                                </Typography>
                            ...
                            <Box className={styles.cycleItemDetails}>
                              <Box className={styles.cycleDetailItem}>
                                <Typography className={styles.cycleDetailLabel}>
                                  Số ngày hành kinh:
                                </Typography>
                                <Typography className={styles.cycleDetailValue}>
                              ...
                              <Box className={`${styles.cycleReminderBadge} ${getReminderStatusInfo(cycle).className}`}>
                                {getReminderStatusInfo(cycle).icon}
                                <Typography className={styles.cycleReminderText}>
                                  {getReminderStatusInfo(cycle).text}
                                </Typography>
                              </Box>
                            </Box>
                        ...
                        <Box className={styles.paginationContainer}>
                          <Box className={styles.paginationInfo}>
                            <Typography className={styles.paginationText}>
                              Hiển thị {startIndex + 1}-{Math.min(endIndex, menstrualCycles.length)} 
                          ...
                          <Box className={styles.paginationControls}>
                            ...
                            >
                              <ChevronUp className={styles.paginationIcon} style={{ transform: 'rotate(-90deg)' }} />
                              Trước
                            </button>
                    ...
                    <Box className={styles.emptyCycleList}>
                      ...
                      <Typography className={styles.emptyCycleDescription}>
                        Hãy bấm nút "Khai báo chu kỳ mới" để thêm chu kỳ đầu tiên của bạn
                      </Typography>
                    </Box>
                  )}
                </Card>
                ...
                <Box id="calculation-result" sx={{ marginBottom: 4 }}>
                  <Card className={styles.healthAdviceCard}>
                    <Box className={styles.healthAdviceHeader}>
                      ...
                      <Box>
                        <Typography variant="h6" className={styles.healthAdviceTitle}>
                    ...
                    <Box className={styles.form}>
                      ...
                      <Card className={`${styles.adviceCardRegular} ${styles.adviceCardBlue}`}>
                        <Box className={styles.adviceCardHeader}>
                          <Box className={styles.adviceCardIconRegular}>
                            <Calendar className={styles.adviceCardIconRegularIcon} />
                          </Box>
                        ...
                        <List className={styles.adviceCardList}>
                          <ListItem className={styles.adviceCardListItem}>
                            Ngày bắt đầu: {formatDate(calculationResult.startDate)}
                          </ListItem>
                          <ListItem className={styles.adviceCardListItem}>
                            Ngày kết thúc dự kiến: {formatDate(calculationResult.endDate)}
                          </ListItem>
                          <ListItem className={styles.adviceCardListItem}>
                            Độ dài kỳ kinh: {calculationResult.periodLength} ngày
                          </ListItem>
                          <ListItem className={styles.adviceCardListItem}>
                            Độ dài chu kỳ: {calculationResult.cycleLength} ngày
                          </ListItem>
                        </List>
                      </Card>

                      {/* Dự đoán chu kỳ tiếp theo */}
                      <Card className={`${styles.adviceCardRegular} ${styles.adviceCardPink}`}>
                        <Box className={styles.adviceCardHeader}>
                          <Box className={styles.adviceCardIconRegular}>
                            <TrendingUp className={styles.adviceCardIconRegularIcon} />
                          </Box>
                          <Typography variant="subtitle1" className={styles.adviceCardTitleRegular}>
                            Dự đoán chu kỳ tiếp theo
                          </Typography>
                        ...
                        <List className={styles.adviceCardList}>
                          <ListItem className={styles.adviceCardListItem}>
                            Ngày bắt đầu dự kiến: {formatDate(calculationResult.nextPeriodDate)}
                          </ListItem>
                          <ListItem className={styles.adviceCardListItem}>
                            Còn khoảng: {Math.ceil((calculationResult.nextPeriodDate - new Date()) / (1000 * 60 * 60 * 24))} ngày
                          </ListItem>
                        </List>
                      </Card>

                      {/* Thời kỳ rụng trứng */}
                      <Card className={`${styles.adviceCardRegular} ${styles.adviceCardGreen}`}>
                        <Box className={styles.adviceCardHeader}>
                          <Box className={styles.adviceCardIconRegular}>
                            <Heart className={styles.adviceCardIconRegularIcon} />
                          </Box>
                          <Typography variant="subtitle1" className={styles.adviceCardTitleRegular}>
                        ...
                        <List className={styles.adviceCardList}>
                          <ListItem className={styles.adviceCardListItem}>
                            Ngày rụng trứng dự kiến: {formatDate(calculationResult.ovulationDate)}
                          </ListItem>
                          <ListItem className={styles.adviceCardListItem}>
                            Thời kỳ thụ thai: {formatDate(calculationResult.fertilityWindow.start)} - {formatDate(calculationResult.fertilityWindow.end)}
                          </ListItem>
                          <ListItem className={styles.adviceCardListItem}>
                            Thời gian thụ thai kéo dài: {Math.ceil((calculationResult.fertilityWindow.end - calculationResult.fertilityWindow.start) / (1000 * 60 * 60 * 24)) + 1} ngày
                          </ListItem>
                        </List>
                      </Card>
                    </Box>
                    ...
                    <Box className={styles.healthAdviceNote}>
                      <b>Lưu ý:</b> Đây chỉ là kết quả tính toán dự đoán. Kết quả thực tế có thể khác do nhiều yếu tố ảnh hưởng. 
                      Để có dữ liệu chính xác hơn, hãy chọn "Bạn đang tính chỉ số cho chính mình?" và lưu thông tin vào hồ sơ.
                    </Box>
                  </Card>
                </Box>
              )}
              ...
              <Box className={styles.healthAdviceSection}>
                <Card className={styles.healthAdviceCard}>
                  ...
                  {advice.map((item, index) => (
                    <Card key={index} className={`${styles.adviceCardRegular} ${styles[`adviceCard${item.color.charAt(0).toUpperCase() + item.color.slice(1)}`]}`}>
                      <Box className={styles.adviceCardHeader}>
                        <Box className={styles.adviceCardIconRegular}>
                          {item.icon}
                        </Box>
